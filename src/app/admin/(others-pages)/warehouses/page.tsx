...
export default function WarehousePage() {
  ...
  const handleSubmit = async () => {
    ...
    const url = isEdit
      ? `http://localhost:1337/api/warehouses/${formData.documentId}`
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(resJson?.error?.message || `Failed to save warehouse [${res.status}]`);
        return;
      }

      toast.success(isEdit ? "Warehouse updated!" : "Warehouse created!");
      setFormOpen(false);
      fetchWarehouses(currentPage); // Maintain current page after edit
    } catch (err) {
      console.error("Unexpected error saving warehouse:", err);
